...
Cascade, cheapest first: parse, identity filter, fragment-diff reduction,
builtin rule replay, QED, SQLSolver, then VeriEQL refutation of survivors and
cross-check of proven findings. Acceptance is PROVEN_EQUIVALENT only; bounded
non-refutation is evidence, never a proof.
"""
...
def verify_bundles(
    ...
    *,
    ...
    builtin: BuiltinVerifierBackend | None = None,
    solver: SqlSolverBackend | None = None,
    qed: QedBackend | None = None,
    refuter: VeriEqlBackend | None = None,
    only: set[str] | None = None,
    report_file: Path | None = None,
    log: Logger | None = None,
) -> dict:
    """Verify every candidate in every bundle; returns counts and any alarms."""
    log = log or (lambda _message: None)
    builtin = builtin or BuiltinVerifierBackend()

    rows: list[dict] = []
    for metadata_path in sorted(bundles_dir.glob("*/metadata.json")):
        ...
        for entry in metadata.get("candidates", []):
            ...
            row.update(
                verify_candidate(
                    original_sql, candidate_sql, constraints, dialect,
                    builtin=builtin, solver=solver, qed=qed, refuter=refuter,
                )
            )
            rows.append(row)
            log(f"{row['bucket']:>12}  {row['model']}/{row['candidate']}")
            if report_file is not None and len(rows) % 20 == 0:
                write_report(report_file, rows, dialect=dialect, solver=solver,
                             qed=qed, refuter=refuter, partial=True)

    if report_file is not None:
        write_report(report_file, rows, dialect=dialect, solver=solver,
                     qed=qed, refuter=refuter, partial=False)
    alarms = [row for row in rows if row.get("crosscheck_alarm")]
    return {
        "candidate_count": len(rows),
        "buckets": dict(sorted(Counter(row["bucket"] for row in rows).items())),
        "alarms": [f"{row['model']}/{row['candidate']}" for row in alarms],
        "results": rows,
    }
...
def verify_candidate(
    ...
    *,
    ...
    # Provers run on the fragment-diff reduction when one applies: proving the
    # differing CTE bodies equivalent proves the full pair by congruence.
    ...
    builtin_result = builtin.verify(prover_original, prover_candidate, constraints, dialect=dialect)
    proven = None
    ...
    if proven is None and qed is not None:
        qed_result = qed.verify(prover_original, prover_candidate, constraints, dialect=dialect)
        if qed_result.status == VerificationStatus.PROVEN_EQUIVALENT:
            proven = {"prover": "qed", "reason": qed_result.reason}
        else:
            solver_note = {**solver_note, "qed_status": qed_result.status.value,
                           "qed_reason": qed_result.reason}

    if proven is None and solver is not None:
        solver_result = solver.verify(prover_original, prover_candidate, constraints,
                                      dialect=dialect)
        if solver_result.status == VerificationStatus.PROVEN_EQUIVALENT:
            ...
            solver_note = {**solver_note, "solver_status": solver_result.status.value,
    ...
    if proven is not None:
        ...
        if refuter is not None:
            ...
            row["crosscheck_alarm"] = crosscheck.status == VerificationStatus.NOT_EQUIVALENT
        return row
    ...
    if refutation.status == VerificationStatus.NOT_EQUIVALENT:
        return {"bucket": "refuted", "reason": refutation.reason,
...
def merge_reports(report_paths: list[Path], report_file: Path | None = None) -> dict:
    """Merge per-prover reports into the best verdict per candidate.

    A proven-vs-refuted disagreement becomes a ``conflict`` (a soundness
    alarm); otherwise the highest-precedence verdict wins.
    """
    ...
    for path in report_paths:
        ...
        for row in report.get("results", []):
            ...
            if existing is None:
                merged[key] = dict(row)
                continue
            if {existing["bucket"], row["bucket"]} == {"proven", "refuted"}:
    ...
    return {
        "candidate_count": len(rows),
        "buckets": dict(sorted(buckets.items())),
        "conflicts": [f"{m}/{c}" for m, c in conflicts],
    }
