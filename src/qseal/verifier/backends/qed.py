...
class QedBackend:
        ...
        if self.parser_jar is None or self.prover_path is None:
            return self._result(
                ...
                reason=(
                    ...
                    "(QSEAL_QED_PARSER_JAR / QSEAL_QED_PROVER)."
                ),
            )
    ...
    def _run_toolchain(self, case_sql: str) -> tuple[VerificationStatus, str]:
        with tempfile.TemporaryDirectory(prefix="qseal-qed-") as temp_dir:
...
def _qed_case(
    ...
    for table_name in sorted(schema):
        ...
        lines = []
        for column in sorted(columns) or ["qseal_id"]:
...
def _option(value: str | Path | None, *env_names: str) -> Path | None:
